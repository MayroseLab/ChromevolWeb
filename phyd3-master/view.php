...
<head>
    ...
    <script>
        ...
        function loadTree() {
            ...
            var ftreeN = "<?php echo ctype_xdigit($_GET['treeNum']) ? $_GET['treeNum'] : ''?>".trim();
            var ftreeType = "<?php echo ctype_xdigit($_GET['treeType']) ? $_GET['treeType'] : ''?>".trim();
            var fmodel = "<?php echo $_GET['model'] ? $_GET['model'] : ''?>".trim();
            <?php if ($_GET['f'] == 'xml_ml') { ?>
                //d3.xml("submissions/"+fid, function(xml) {
                d3.xml("results/" + fid + "/ChromEvol_Tree_" + ftreeN + "/" + fmodel + "/mlAncestors_phylo_edit", function(xml) {
                    d3.select("#phyd3").text(null);
                    var tree = phyd3.phyloxml.parse(xml);
                    phyd3.phylogram.build("#phyd3", tree, opts);
                });
			<?php } else if ($_GET['f'] == 'xml_post') { ?>									  
                d3.xml("results/" + fid + "/ChromEvol_Tree_" + ftreeN + "/" + fmodel + "/posteriorAncestors_phylo_edit", function(xml) {
			...
			<?php } else if ($_GET['f'] == 'input') { ?>												 
               d3.xml("results/" + fid + "/ChromEvol_Tree_" + ftreeN + "/tree_" + ftreeN, function(xml) {
